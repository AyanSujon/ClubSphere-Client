...
//           />
//           {errors.clubName && <p className="text-red-500 mt-1">{errors.clubName.message}</p>}
//         </div>
...
//             className="input input-bordered w-full"
//           />
//           {errors.bannerImage && <p className="text-red-500 mt-1">{errors.bannerImage.message}</p>}
//         </div>

//         {/* Row 3: Category + Membership Fee */}
...
//             <input
//               type="number"
//               {...register("membershipFee", { required: "Membership Fee is required", min: 0 })}
//               className="input input-bordered w-full"
//             />
//             {errors.membershipFee && <p className="text-red-500 mt-1">{errors.membershipFee.message}</p>}
//           </div>
//         </div>
...
const EditClub = () => {
  ...
  // Get clubId from URL and managerEmail from Link state
  const { clubId } = useParams();
  const location = useLocation();
  const { managerEmail } = location.state || {};

  // Fetch single club data
  const { data: clubData, isLoading, isError } = useQuery({
    queryKey: ["singleClub", clubId],
    queryFn: async () => {
      const response = await axiosSecure.get(`/clubs/${clubId}?email=${managerEmail}`);
  ...
  return (
    <div className="max-w-3xl mx-auto p-6 bg-white rounded-lg shadow-lg mt-10">
      ...
      <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
        ...
        {/* Submit */}
        <div className="mt-4">
          <button type="submit" className="btn btn-primary w-full">Update Club</button>
        </div>
      </form>
    </div>
  );
