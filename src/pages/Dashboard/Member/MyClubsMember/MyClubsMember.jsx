...
const MyClubsMember = () => {
    ...
    // Fetch clubs using TanStack Query
    const { data: clubs = [], isLoading, isError, error } = useQuery({
        queryKey: ['myClubs', userEmail],
        queryFn: async () => {
            const response = await axiosSecure.get(`/member/my-clubs?userEmail=${userEmail}&role=${role}`);
            return response.data;
        },
    });

    if (isLoading) return <p>Loading clubs...</p>;
    if (isError) return <p>Error fetching clubs: {error.message}</p>;
            ...
            <div className="overflow-x-auto">
                <table className="table table-zebra w-full">
                    ...
                    <tbody>
                        {clubs.map((club) => (
                            <tr key={club._id}>
                                <td>{club.clubName || club.category}</td>
                                <td>{club.location}</td>
                                <td>
                                    {club.status === "active" ? (
                                        <span className="badge badge-success gap-2 flex items-center">
                                ...
                                <td>{new Date(club.joinedAt).toLocaleString('en-US', { hour12: true })}</td>
                                <td>
                                    <Link to={`/dashboard/member/my-clubs/${club._id}`} className="btn btn-sm btn-primary flex items-center gap-1">
                                        <FaEye /> View
                                    </Link>
                                </td>
                            </tr>
